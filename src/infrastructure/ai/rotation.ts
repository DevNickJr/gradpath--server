...
export const PROVIDERS = ["groq", "gemini", "openai"] as const;
...
export const TIERS = [
  {
    ...
    models: {
      ...
      openai: "gpt-4o" // or your primary OpenAI model
    }
  },
  {
...
export function recordSuccess() {
  logger.info(`Content Generation Success ${getCurrentModel()}`)
}
...
export function recordFailure() {
  ...
  if (currentProviderIndex < PROVIDERS.length - 1) {
    ...
    console.warn(
      `[Rotation] ${failedProvider} (${failedModel}) failed. Moving to next provider in same tier: ${PROVIDERS[currentProviderIndex]} (${getCurrentModel()})`
    );
  } else {
